"""
Generates sample data, applies the Feast registry (including permissions),
and writes features into the online store.
...
def main() -> None:
    ...
    print("Applying Feast registry (features + permissions)...")
    sys.path.insert(0, REPO_DIR)
    from feast import FeatureStore
    from features import customer, customer_profile
    from permissions import admin_permission, reader_permission

    store = FeatureStore(repo_path=REPO_DIR)
    store.apply([customer, customer_profile, admin_permission, reader_permission])
